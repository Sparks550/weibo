...
class UsersController extends Controller
{
    ...
    {
        $report = new Report();
        $content_id = $report->content_id = $id;
        return view('users/report', ['id' => $content_id]);
    ...
    public function receiveReport(Request $request)
    {
        ...
        $report->report_content = $request->reportContent;
        $report->content_id = $request->content_id;
        $report->is_report = 0;

        $report->save();

        session()->flash('success', '系统已经接受到您的反馈，感谢您做出的贡献！');
        return redirect('/');
    }
